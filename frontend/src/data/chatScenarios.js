...
export const chatScenarios = {
  ...
  emergency_water_owner_responsibility: {
    text: 'Повреждение внутриквартирного оборудования — ответственность собственника. Вам нужно возместить ущерб соседям, если он есть.\n\nРекомендуем решить спор без суда в добровольном порядке. Это поможет избежать судебных издержек и сохранить нормальные отношения с соседями.',
    options: [
      { label: 'Решить спор во внесудебном порядке', value: 'emergency_water_owner_responsibility_no_judge' },
    ]
  },

  emergency_water_owner_responsibility_no_judge: {
  ...
  emergency_water_checklist: {
    text: 'Перенаправляю вас в раздел Алгоритм судебного сопровождения. В этом разделе расписаны пукнты, которые необходимы для подачи обращения в УК. Проверьте, что вы выполнили все пукнты.',
    options: [
